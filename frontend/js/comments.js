...
// 返信生成（特定のコメントに対して）
async function generateReply(apiKey, replyPersona, targetComment, articleText, allComments) {
  ...
  for (let i = 0; i < totalPersonas; i++) {
    ...
    if (shouldReply && allComments.length > 0) {
      ...
      for (const target of replyTargets) {
        const replyData = await generateReply(apiKey, persona, target, articleText, allComments);
        // 返信を対象コメントに追加
