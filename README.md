...
生成されるHTMLは選択した傾向に応じて各掲示板のデザインを再現：

| 傾向 | デザイン特徴 | AI要約 |
|---|---|---|
| 🤖 Reddit風 | upvote矢印（▲▼）、`u/` 表記、Redditライクな配色 | なし |
| 🗿 2ちゃんねる風 | 番号付きレス、MS Pゴシック風フォント、緑基調 | なし |
| 😤 Yahooコメント風 | 「そう思う/そう思わない」ボタン、Yahoo風レイアウト | ✅ あり |

ペルソナは感情的な人と冷静な人の**両方を必ず混在**させ、
互いに反発して喧嘩腰の議論が生まれるよう設計されています。
...
3. **ペルソナ設定**:
   - 人数 (1〜100人、デフォルト10)
   - 議論の傾向:
     - 🤖 **Reddit風** — 知的ながらカジュアル。upvote争い。皮肉・ミーム交じり
     - 🗿 **2ちゃんねる風** — 匿名の無法地帯。煽り・スラング・たまに鋭い考察
     - 😤 **Yahooコメント風**（デフォルト）— 説教・上から目線・ご意見番。AI要約つき
   - 対象地域（複数選択可、9地域から。デフォルト: 東アジア・東南アジア・中東・欧州・北米）
4. **免責事項確認 → 生成開始**
5. 生成中のコメントを**ライブ表示**で確認
6. 完了後、**📋 HTMLをコピー** または **💾 HTMLをダウンロード**（掲示板別デザイン）
...
| モデル | 用途 | 状態 |
|---|---|---|
| Gemini 3.1 Flash-Lite | ペルソナ生成・コメント生成（本体） | ✅ 利用可能 |
| Gemini 2.5 Flash-Lite | Google検索グラウンディング（背景情報） | ✅ 利用可能 |
| Claude Haiku 4.5 | （将来対応） | 🔜 準備中 |
| GPT-5.6 Luna | （将来対応） | 🔜 準備中 |
